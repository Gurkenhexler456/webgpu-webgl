...
export const EngineHelper = {
     ...
     */
    create: function (canvas, render_resolution = new Extents2D(1280, 720), backend = EngineBackend.BACKEND_WEBGL_2) {
        
        backend = request_backend(backend);

        if(backend === EngineBackend.BACKEND_WEBGPU) {
            return new SolarEngine_WebGPU(canvas, render_resolution); 
        }
        else {
            return new SolarEngine_WebGL(canvas, render_resolution);
        }
    }
}